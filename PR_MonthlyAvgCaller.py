...
for i in range(startYear, endYear+1):
    ...
    for x in range(startMonth, endMonth+1):
        ...
        month.getMonths(x)
        fileOut = GlobalDateObj.ws.split('\\')[-1][:-4]
        RasterStats(month, rastersOut + '\\' + fileOut + "_" + str(i) + "_" + str(x) + ".img", GlobalDateObj)
...
if len(Tables_List) != 0:
    ...
    arcpy.AddMessage("calculations are complete, please see " + table.split("_")[0] + "_" + str(startYear) + "_" + str(startMonth) + " in " + output_gdb + " to see final results.")
else:
